...
~/xidel  --variable 'user,pass' 'https://moodle.uni-luebeck.de/' -f 'form(//form, {"username": $user, "password": $pass})' \
   ...
   -e 'for $user in //a[contains(@href, "assignsubmission_file")] return $user/ancestor::tr[1]/((.//a[contains(@href, "user/view")])[last()])/x"{extract($user/@href, "(([0-9]+)/[^/]+[.][a-z]+)([?].*)?$", 2)} {extract(@href, "id=([0-9]+)", 1)} {.}"' \
...
echo copying to alan: submission/*
