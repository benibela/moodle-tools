...
#export moodletmppath=/tmp
#export submissionspath=/tmp/submissions
#export resultpath=/tmp/grader/result

moodle --verbose --variable moodletmppath,resultpath,exercise,submissionspath -e '
  xquery version "3.1-xidel";
  $allexercises := tokenize($exercise, ","),
  $allsubmissions := $allexercises ! { "id": ., 
                                           "submissions": distinct-values(("/old" || ., "/new" || .) 
                                                          ! file:read-text-lines($submissionspath || .))  },
  $failed := file:list($resultpath || "/failed"),
  $failedurlregex := join( $failed ! ("submission_files/" || .), "|"),
  $failedsubmissions := $allsubmissions ! {"id": ?id, 
                                            "submissions": ?submissions[matches(., $failedurlregex)]}
                                           [count(?submissions) > 0]
 ' -f '$failedsubmissions ! x"https://moodle.uni-luebeck.de/mod/assign/view.php?id={?id}&action=grading"' \
  -f 'xquery version "3.1-xidel";
      let $form := (//form[contains(@action, "view.php") and not(contains(@class, "batch"))])[1]
      return
      form($form, for $downloadlinks in $form//a[contains(@href, "submission_files")]
                  let $submissionid := extract($downloadlinks/@href, "submission_files/([0-9]+)", 1)
                  where $submissionid = $failed
                  return {$downloadlinks/ancestor::tr[1]//textarea/@name: file:read-text($resultpath || "/failed/" || $submissionid)}
          )
   ' -e '//title'
   
exit   

      let $id := extract($id, "id=([0-9]+)", 1)
      let $submissions :=  $failedsubmissions[?id = $id]?submissions
  
  for $file in  ! tokenize(., $line-ending) [normalize-space()]
