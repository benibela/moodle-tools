...
~/xidel  $( (ls *.sheet; ls *.tex) | sort | tail -1) --variable 'course,user,pass' -e 'sheet := $raw, hour := 10, $allow-file-upload := true(), uploadFilename := replace($url, "[.]tex", ".pdf"), slang := extract($sheet, "class\[(.*)\]\{article", 1), snumber := extract($sheet, "insertsheetnumber\{(.*)\}", 1), texdeadline := extract($sheet, "insertdeadline\{(.*)\}", 1 ), sdeadline := if (contains($texdeadline, ",")) then tokenize($texdeadline, ",") ! extract(., "[0-9]*") else reverse(tokenize(xs:string(parse-date(normalize-space(replace($texdeadline, "[^0-9a-zA-Z]", " ", "g")), "d mmmm yyyy" )), "-") ! extract(., "[1-9][0-9]*")) ,  $description := $title := if ($slang eq "english") then x"Exercise sheet {$snumber}" else x"Übungsblatt {$snumber}", $assignmenttitle := $title (: || if ($slang eq "english") then " (results)" else " (Ergebnisse)" :), $spoints := sum(tokenize(if (contains($sheet, "begin{homework}")) then substring-after($sheet, "begin{homework}") else $sheet, $line-ending) !  extract(., "^[^%]*credits=([^\],%]*)", 1) ! tokenize(., "[a-zA-Z ]+") [.] ! number())  '  \
